...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ROUTE_STORAGE_KEY = 'pepeweb_logica_route';

    function getCurrentRoute() {
        if (IS_FILE_PROTOCOL) return window.location.hash || '#/logica';
        const fromState = history.state && history.state.route;
        if (fromState && typeof fromState === 'string') return fromState;
        const fromSession = sessionStorage.getItem(ROUTE_STORAGE_KEY);
        if (fromSession && typeof fromSession === 'string') return fromSession;
        return '#/logica';
    }

    function navigateRoute(route, { push = true } = {}) {
        const safeRoute = (typeof route === 'string' && route.startsWith('#/logica')) ? route : '#/logica';
        if (IS_FILE_PROTOCOL) {
            if (window.location.hash !== safeRoute) window.location.hash = safeRoute;
            else router();
            return;
        }
        sessionStorage.setItem(ROUTE_STORAGE_KEY, safeRoute);
        const cleanUrl = window.location.pathname + window.location.search;
        if (push) history.pushState({ route: safeRoute }, '', cleanUrl);
        else history.replaceState({ route: safeRoute }, '', cleanUrl);
        router();
    }

    (function bootstrapRoute() {
        if (IS_FILE_PROTOCOL) {
            if (!window.location.hash) window.location.hash = '#/logica';
            return;
        }

        const fromHash = (window.location.hash && window.location.hash.startsWith('#/logica')) ? window.location.hash : null;
        const fromSession = sessionStorage.getItem(ROUTE_STORAGE_KEY);

        if (fromHash) {
            sessionStorage.setItem(ROUTE_STORAGE_KEY, fromHash);
            history.replaceState({ route: fromHash }, '', window.location.pathname + window.location.search);
            return;
        }

        if (!history.state || !history.state.route) {
            const initial = (fromSession && fromSession.startsWith('#/logica')) ? fromSession : '#/logica';
            history.replaceState({ route: initial }, '', window.location.pathname + window.location.search);
        }
    })();
    ...
    function openSidebarSubmenu(themeId, anchorBtn) {
        ...
        const activeRelMatch = getCurrentRoute().match(new RegExp(`#\\/logica\\/T${themeId}\\/(rel\\d+)`));
        ...
        const render = () => {
            ...
            relEntries.forEach(([relId, relData]) => {
                ...
                btn.onclick = () => {
                    closeSidebarSubmenu();
                    navigateRoute(`#/logica/T${themeId}/${relId}`);
    ...
    function openSidebarSubmenuEx(anchorBtn) {
        ...
        const activeExamMatch = getCurrentRoute().match(/#\/logica\/(P1|P2|C)\b/);
    ...
    async function router() {
        ...
        if (hash.startsWith('#/logica')) {
            ...
            if (parts.length === 2) {
                ...
                let exData = findExerciseIdByName(exName);
                
                // Fallback: Support Declarative/Applicative variants
                if (!exData) {
                    // 1. Try finding by adding _auto (User wants Declarative (base), Data has Applicative (base_auto))
                    let candidate = findExerciseIdByName(exName + '_auto');
                    if (candidate) {
                        exData = { ...candidate, name: exName, defaultMethod: 'Declarativa' };
                    } else {
                        // 2. Try finding by removing _auto (User wants Applicative (base_auto), Data has Declarative (base))
                        const base = exName.replace('_auto', '');
                        candidate = findExerciseIdByName(base);
                        if (candidate) {
                            exData = { ...candidate, name: exName, defaultMethod: 'Aplicativa' };
                        }
                    }
                }
    ...
    function renderSidebar() {
        ...
        themesList.forEach((themeId) => {
            ...
            btn.onclick = () => {
                ...
                if (relCount > 1) {
                    ...
                    closeSidebarSubmenu();
                    navigateRoute(`#/logica/T${themeId}`);
                    if (window.matchMedia('(max-width: 1024px), (hover: none) and (pointer: coarse)').matches) {
                        closeMobileSidebar();
                    }
                }
        ...
        exBtn.onclick = () => {
            // closeSidebarSubmenu(); // REMOVED to allow switching animation
            document.querySelectorAll('.theme-btn').forEach(b => b.classList.remove('active'));
    ...
    function renderExamsMenu(animate = true) {
        ...
        exams.forEach(ex => {
            ...
            `;
            btn.onclick = () => { navigateRoute(`#/logica/${ex.id}`); };
            frag.appendChild(btn);
        });
    ...
    function renderRelations(themeId, animate = true) {
        ...
        relEntries.forEach(([relId, relData]) => {
            ...
            btn.onclick = () => {
                ...
                navigateRoute(`#/logica/${prefix}${themeId}/${relId}`);
            };
    ...
    function showMethodSelectionModal(themeId, relationId, exData) {
        if (!methodModal) {
            methodModal = document.createElement('div');
            methodModal.className = 'modal-backdrop';
            methodModal.innerHTML = `
                <div class="modal-content liquidGlass-wrapper">
                    <div class="liquidGlass-effect"></div>
                    <div class="liquidGlass-tint"></div>
                    <div class="liquidGlass-shine"></div>
                    <div class="liquidGlass-text" style="position:relative;z-index:10;">
                        <h3 class="modal-title">Selecciona el Método</h3>
                        <p class="modal-subtitle">¿Cómo quieres resolver este ejercicio?</p>
                        <div class="method-options">
                            <button class="method-btn" id="btn-method-decl">
                                <span class="method-icon">🧠</span>
                                <span class="method-name">Declarativa</span>
                                <span class="method-desc">Paso a paso con Isar</span>
                            </button>
                            <button class="method-btn" id="btn-method-app">
                                <span class="method-icon">⚡</span>
                                <span class="method-name">Aplicativa</span>
                                <span class="method-desc">Automática con apply</span>
                            </button>
                        </div>
                        <button class="icon-btn" style="position:absolute;top:-1.5rem;right:-1.5rem;background:transparent;border:none;color:var(--c-text-muted);cursor:pointer;" id="btn-modal-close">
            ...
            const close = () => methodModal.classList.remove('visible');
            methodModal.querySelector('#btn-modal-close').onclick = close;
            methodModal.onclick = (e) => {
                if (e.target === methodModal) close();
            };
        }

        const btnDecl = methodModal.querySelector('#btn-method-decl');
        const btnApp = methodModal.querySelector('#btn-method-app');

        // Logic to determine IDs
        let baseName = exData.name;
        if (baseName.endsWith('_auto')) {
            baseName = baseName.replace('_auto', '');
        }

        const declId = baseName;
        const appId = baseName + '_auto';

        btnDecl.onclick = () => {
            methodModal.classList.remove('visible');
            const prefix = (themeId.startsWith('P') || themeId === 'C') ? '' : 'T';
            navigateRoute(`#/logica/${prefix}${themeId}/${relationId}/${declId}`);
        };

        btnApp.onclick = () => {
            methodModal.classList.remove('visible');
            const prefix = (themeId.startsWith('P') || themeId === 'C') ? '' : 'T';
            navigateRoute(`#/logica/${prefix}${themeId}/${relationId}/${appId}`);
        };
    ...
    function renderThemeEjercicios(themeId, relationId, animate = true) {
        ...
        exEntries.forEach(([exKey, exData]) => {
            ...
            card.onclick = () => {
                showMethodSelectionModal(themeId, relationId, exData);
            };
    ...
    function openIsabelleView(exMetadata, animate = true) {
        ...
        if (!proofData) {
            ...
            // Show placeholder in UI instead of failing silently
            state.currentExercise = { steps: [] }; // Empty dummy
            state.currentStep = -1;
            ...
            DOM.isabelle.codeContainer.innerHTML = '<div style="padding:2rem;text-align:center;color:var(--c-text-muted);">Esta versión del ejercicio aún no está disponible.</div>';
            ...
            DOM.isabelle.btnNext.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>'; // Reset icon
    ...
    function exitExerciseView(updateHistory = true) {
        if (updateHistory) navigateRoute('#/logica', { push: false });
...

{
  ...
  "steps": [
    ...
    function admAddStepUI(stepData = {}, insertAfterElement = null) {
        ...
        header.className = 'adm-step-header';
        
        const title = document.createElement('strong');
        title.textContent = `Paso ${idx}`;
        
        const actions = document.createElement('div');
        actions.className = 'adm-step-actions';
        
        const btnUp = document.createElement('button');
        btnUp.className = 'adm-step-btn';
        btnUp.title = 'Mover Arriba';
        btnUp.innerHTML = admCreateIcon('<path d="M18 15l-6-6-6 6"/>');
        btnUp.onclick = () => admMoveStep(card, -1);
        
        const btnDown = document.createElement('button');
        btnDown.className = 'adm-step-btn';
        btnDown.title = 'Mover Abajo';
        btnDown.innerHTML = admCreateIcon('<path d="M6 9l6 6 6-6"/>');
        btnDown.onclick = () => admMoveStep(card, 1);

        const btnDup = document.createElement('button');
        btnDup.className = 'adm-step-btn';
        btnDup.title = 'Duplicar';
        btnDup.innerHTML = admCreateIcon('<rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>');
        btnDup.onclick = () => admDuplicateStep(card);

        const btnAdd = document.createElement('button');
        btnAdd.className = 'adm-step-btn';
        btnAdd.title = 'Insertar Nuevo Debajo';
        btnAdd.innerHTML = admCreateIcon('<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>');
        btnAdd.onclick = () => admAddStepUI({}, card);

        const btnCollapse = document.createElement('button');
        btnCollapse.className = 'adm-step-btn';
        btnCollapse.title = 'Colapsar/Expandir';
        btnCollapse.innerHTML = admCreateIcon('<line x1="5" y1="12" x2="19" y2="12"/>'); // Minus (Start expanded)
        btnCollapse.onclick = () => {
            card.classList.toggle('collapsed');
            const isCollapsed = card.classList.contains('collapsed');
            btnCollapse.innerHTML = isCollapsed 
                ? admCreateIcon('<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>') // Plus
                : admCreateIcon('<line x1="5" y1="12" x2="19" y2="12"/>'); // Minus
        };

        const btnDel = document.createElement('button');
        btnDel.className = 'adm-step-btn delete';
        btnDel.title = 'Eliminar';
        btnDel.innerHTML = admCreateIcon('<line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>');
        btnDel.onclick = () => { 
            // if(confirm('¿Borrar este paso?')) { // Removed confirm for faster flow as requested "perfect/ideal"
                card.remove(); 
                admRenumberSteps(); 
            // }
        };

        actions.append(btnUp, btnDown, btnDup, btnAdd, btnCollapse, btnDel);
        header.append(title, actions);
        
        // Body container
        const body = document.createElement('div');
        body.className = 'adm-step-body';
        body.innerHTML = `
        ...
        if (insertAfterElement && insertAfterElement.parentNode === adminDOM.stepsContainer) {
            adminDOM.stepsContainer.insertBefore(card, insertAfterElement.nextSibling);
        } else {
        ...
        // Auto-focus code if inserted manually (not during initial load)
        if (insertAfterElement) {
             const ta = card.querySelector('.step-code');
             if(ta) ta.focus();
        }
    }

    function admMoveStep(card, direction) {
        if (direction === -1) { // Up
            const prev = card.previousElementSibling;
            if (prev) {
                adminDOM.stepsContainer.insertBefore(card, prev);
                admRenumberSteps();
            ...
            if (next) {
                adminDOM.stepsContainer.insertBefore(next, card);
                admRenumberSteps();
                card.scrollIntoView({ behavior: 'smooth', block: 'center' });
            }
        }
    ...
    function admDuplicateStep(card) {
        const code = card.querySelector('.step-code').value;
        const explanation = card.querySelector('.step-explanation').value;
        const activeHyp = card.querySelector('.step-hyp').value.split(',').map(s=>s.trim()).filter(Boolean);
        const highlights = card.querySelector('.step-highlights').value.split(',').map(s=>s.trim()).filter(Boolean);
        
        admAddStepUI({ code, explanation, activeHyp, highlights }, card);
    }

    function admRenumberSteps() {
        Array.from(adminDOM.stepsContainer.children).forEach((card, i) => {
            const title = card.querySelector('.adm-step-header strong');
            if (title) title.textContent = `Paso ${i + 1}`;
        });
    }
